# этот скрипт считает статистику относительно встречаемости причастных оборотов для человеческих
# и ИИ текстов
...
with open('AI_Human_Clauses.csv', 'r', encoding='utf8') as f:
    with open('clause_stats.txt', 'w', encoding='utf8') as f_res:
        ...
        for row in reader:
            ...
            if row['generated'] == '1.0':
                list_ai.append(int(row['clause_count'])/int(row['word_count']))
            else:
                list_human.append(int(row['clause_count'])/int(row['word_count']))
